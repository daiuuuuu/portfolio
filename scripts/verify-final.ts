...
async function main() {
  ...
  // 20 rapid notches
  const samples = []
  for(let i=0;i<20;i++){await page.mouse.wheel(0,100);await page.waitForTimeout(16);samples.push(Math.round(await page.evaluate('window.scrollY')))}
  const mono = samples.every((v,i)=>i===0||v>=samples[i-1])
  console.log('monotonic:', mono, 'range:', samples[0], '→', samples[samples.length-1])
  
  // inertia
  const b4 = samples[samples.length-1]
  await page.waitForTimeout(1500)
  const after = await page.evaluate('window.scrollY')
  console.log('inertia:', Math.round(after - b4), 'px')
